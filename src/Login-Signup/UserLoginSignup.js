...
const UserLoginSignup = () => {
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/signup/signup`, SignUp, {
      ...
      try{
        const emailResponse = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/mailSend/sendMail`, {email: SignupEmail}, {
          headers: { "Content-Type": "application/json" },
        })
  ...
  const handleLoginSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/login/login`, LogIn, {
        headers: { "Content-Type": "application/json" },
      });
